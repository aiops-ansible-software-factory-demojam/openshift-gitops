...
class Engine:
    ...
    def accept(self, prompt: str, source_execution_id: str, idempotency_key: str) -> tuple[RunRecord, int]:
        ...
        if outcome == "replay":
            ...
            return existing, 200 if existing.terminal else 202
    ...
    def request_cancel(self, run_id: str) -> RunRecord:
        ...
        if run.terminal:
            return run
        return self.store.update(run_id, cancel_requested=1, updated_at=iso(self.clock()))
    ...
    def _provision(self, run_id: str) -> None:
        run = self._touch(run_id, "provisioning")
    ...
    def _prepare(self, run_id: str) -> None:
        ...
        self.client.upload(run.sandbox_name, task_path, f"{self.config.workdir}/task.json")
    ...
    def _run_agent(self, run_id: str) -> None:
        ...
        result = self.client.exec(
            run.sandbox_name,
            ["python3", self.config.wrapper, f"{self.config.workdir}/task.json"],
            timeout,
            self.config.workdir,
        )
    ...
    def _collect(self, run_id: str) -> None:
        ...
        if download.exists():
            for child in sorted(download.rglob("*"), reverse=True):
                if child.is_file() or child.is_symlink():
                    ...
                    child.rmdir()
        self.client.download(run.sandbox_name, f"{self.config.workdir}/out", download)
        report_path = download / "result.json"
        ...
        self._publish(run.id, "workspace", "workspace.tar.gz", archive, "application/gzip")
        events = download / "events.ndjson"
        if events.is_file():
            self._publish(run.id, "events", "events.ndjson", events, "application/x-ndjson")
        if report_path.is_file():
            self._publish(run.id, "report", "result.json", report_path, "application/json")
        self._publish(
            run.id,
            "manifest",
            "manifest.json",
        ...
        self.store.update(
            ...
            agent_exit_code=report.get("agent_exit_code", exit_code)
            if isinstance(report.get("agent_exit_code", exit_code), int) or report.get("agent_exit_code") is None
            else exit_code,
            updated_at=iso(self.clock()),
        )
    ...
    def _complete(self, run_id: str) -> None:
        run = self._require(run_id)
        self._terminal(run_id, "completed", "")
    ...
    def _cleanup(self, run_id: str) -> None:
        ...
        try:
            self.client.delete(run.sandbox_name)
            ...
            return
        if run.sandbox_name in self.client.list_managed():
            self.store.update(run_id, cleanup_state="failed", updated_at=iso(self.clock()))
            return
    ...
    def _reap_orphans(self) -> None:
        ...
        for name in self.client.list_managed():
    ...
    def _publish(self, run_id: str, artifact_id: str, name: str, path: Path, content_type: str) -> None:
        ...
        if b"OPENSHELL_OIDC_CLIENT_SECRET" in data or b"BEGIN PRIVATE KEY" in data:
